...
export function SubscriptionReceipt({
  ...
  const receiptData: {
    ...
    items: [
      {
        id: receipt.id,
        quantity: 1,
        description: `${receipt.plan} Subscription (Valid: ${formatDate(new Date(receipt.startDate))} - ${formatDate(new Date(receipt.endDate))})`,
    ...
    total: receipt.price,
    paymentInstructions: {
      cardDetails: `${receipt.cardType.toUpperCase()} ending in ${receipt.cardNumber.slice(-4)}`,
    },
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="bg-gradient-to-b from-gray-900 to-gray-950 max-w-md w-full rounded-xl p-0 border-none shadow-xl">
        <DialogHeader className="p-4 pb-0">
          <VisuallyHidden>
            <DialogTitle>Subscription Receipt</DialogTitle>
          </VisuallyHidden>
          <h2 className="text-xl font-semibold text-blue-300">SUBSCRIPTION RECEIPT</h2>
        </DialogHeader>
        ...
        <Card className="bg-gray-950/70 backdrop-blur-sm border border-gray-800 text-gray-100">
          <CardHeader className="p-4 pt-2">
            <div className="flex justify-between items-start">
              <div>
                <p className="font-semibold text-xs text-blue-400">{receiptData.companyName}</p>
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-gray-300 min-w-[300px]">
                <thead>
                ...
                <tbody>
                  {receiptData.items.map((item) => (
                    <tr key={item.id} className="border-b border-gray-700">
                      ...
                      <td className="py-1 px-2 whitespace-normal">{item.description}</td>
                      <td className="py-1 px-2 text-right">{item.unitPrice.toFixed(2)}</td>
          ...
          <CardContent className="p-4 pt-0">
            <div className="flex justify-end">
              <div className="w-full">
                <div className="text-right">
                  ...
                  <p className="text-xl font-bold text-white">${receiptTotal.toFixed(2)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs">Paid via: {receiptData.paymentInstructions.cardDetails}</p>
            </div>
          ...
          <CardContent className="p-4 pt-0 text-center text-[10px] text-gray-400">
            ...
            <div className="flex items-center justify-center gap-1">
              ...
              <span>
                Email:{" "}
                <a href={`mailto:${receiptData.companyEmail}`} className="text-blue-400 hover:underline">
        ...
        <DialogFooter className="p-3">
          <Button variant="outline" onClick={() => onOpenChange(false)} className="text-xs text-gray-300 border-gray-600 hover:bg-gray-700 px-3 py-1">
            Close
          </Button>
          {onPrint && (
            <Button variant="default" onClick={onPrint} className="text-xs bg-blue-500 hover:bg-blue-600 text-white px-3 py-1">
              Print
            </Button>
          )}
        </DialogFooter>
